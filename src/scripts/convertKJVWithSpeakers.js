// Enhanced KJV Data Conversion Script with Speaker Detection
// This script converts KJV_books data and includes speaker information
...
// Book metadata mapping (same as before)
const bookMetadata = {
...
// Book themes and characters (same as before)
const bookThemes = {
...
function createVerseWithSpeaker(verseId, bookId, bookName, chapter, verse, verseText) {
...
function convertBookWithSpeakers(bookName, bookData) {
...
function convertKJVDataWithSpeakers() {
  ...
  console.log(`Found ${files.length} books to convert with speaker detection...`)
  ...
  files.forEach((file) => {
    ...
    try {
      ...
      const converted = convertBookWithSpeakers(bookName, bookData)
      if (converted) {
        ...
        console.log(
          `✓ Converted ${bookName} (${converted.book.chapterCount} chapters, ${converted.book.verseCount} verses) - Jesus: ${converted.book.speakerStats.jesusVerses}, God: ${converted.book.speakerStats.godVerses}`,
        )
      }
    } catch (error) {
  ...
  console.log(`\n🎉 Conversion with speaker detection complete!`)
  ...
  console.log(`📖 Narrator verses: ${allVerses.length - totalJesusVerses - totalGodVerses}`)
  console.log(`\nFiles created:`)
  console.log(`- allBooks.json (combined book data with speaker stats)`)
  console.log(`- allVerses.json (all verses with speaker information)`)
  console.log(`- Individual book files (e.g., matthewBook.json, matthewVerses.json)`)
}

// Run the enhanced conversion
convertKJVDataWithSpeakers()
