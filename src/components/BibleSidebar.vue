...
// Sidebar state
const summaryExpanded = ref(false) // accordion collapsed by default
...
<template>
	<div
		class="flex-[1] h-full min-h-0 bg-white/60 rounded-xl shadow-lg p-8 flex flex-col gap-2 overflow-hidden flex-shrink-0 mr-2"
	>
		<!-- Book Summary Section (accordion, collapsed by default) -->
		<div class="flex-shrink-0">
			<button
				type="button"
				class="flex items-center justify-between w-full text-left gap-2 py-1 rounded hover:bg-neutral-20 transition-colors"
				:aria-expanded="summaryExpanded"
				aria-controls="book-summary-content"
				id="book-summary-toggle"
				@click="summaryExpanded = !summaryExpanded"
			>
				<h3 class="text-sm font-bold">Summary</h3>
				<HeroIcons.ChevronDownIcon
					class="w-4 h-4 flex-shrink-0 text-neutral-60 transition-transform duration-200"
			...
			<div
				...
				role="region"
				aria-labelledby="book-summary-toggle"
				:aria-hidden="!summaryExpanded"
				class="overflow-hidden transition-all duration-200"
				:class="summaryExpanded ? 'visible' : 'invisible max-h-0'"
			>
				<p class="text-xs text-gray-700 leading-relaxed pt-2">
